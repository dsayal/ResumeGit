...
def get_winner(elector_dict, outcome_dict):
    """This function calculates the number of electoral votes received by each candidate ("R" or "D") and returns the winning candidate
    and total number of electoral votes that candidate received.

    Args:
        elector_dict (dict): indicate the number of electors per state
        outcome_dict (dict): assigns a hypothetical winning party ("R" or "D") to each state

    Returns:
        tuple : winning candidate and total number of electoral votes that the candidate received 
    """
